/**
 * @file fsk_simple_tx_rx.ino
 * @brief MLR-ModemライブラリのFSKモードにおける基本的な送受信サンプル
 ...
 * 2. loop()関数内で、5秒ごとに "Hello MLR!" というメッセージを送信します (FSKモード)。
...
void setup()
{
  ...
  // 無線通信モードをFSKモードに設定
  modem.SetMode(MLR_ModemMode::FskCmd, false);
...
void loop()
{
  ...
  // --- 送信処理 (例: 5秒ごとに "Hello MLR!" を送信) ---
  static uint32_t lastSendTime = 0;
  if (millis() - lastSendTime > 5000)
  {
    lastSendTime = millis();
    const char *message = "Hello MLR!";
